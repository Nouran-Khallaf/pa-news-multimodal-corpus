...
class RobotsCache:
    ...
    def allowed(self, url: str, *, allow_if_unavailable: bool = False) -> bool:
        parsed = urlparse(url)
        origin = f"{parsed.scheme}://{parsed.netloc}"
        if origin not in self.cache:
            self.cache[origin] = self._load(
                origin, allow_if_unavailable=allow_if_unavailable
            )
    ...
    def _load(
        self, origin: str, *, allow_if_unavailable: bool = False
    ) -> RobotFileParser | None:
        robots_url = origin + "/robots.txt"
        try:
            response = self.crawler._request_raw(
                robots_url, purpose="robots", apply_delay=False
            )
        except CrawlBlocked as exc:
            if allow_if_unavailable:
                self.crawler.logger.warning(
                    "robots.txt unavailable for explicit allowlisted asset origin %s; "
                    "continuing only for asset URLs embedded in captured article HTML: %s",
                    origin, exc,
                )
                return None
            ...
            if allow_if_unavailable:
                self.crawler.logger.warning(
                    "robots.txt request failed for explicit allowlisted asset origin %s; "
                    "continuing only for asset URLs embedded in captured article HTML: %s",
                    origin, exc,
                )
                return None
            ...
            self.crawler.logger.warning(
                "robots unavailable for %s; continuing by configuration", origin
            )
            return None
        ...
        if response.status_code != 200:
            if allow_if_unavailable:
                self.crawler.logger.warning(
                    "robots.txt returned HTTP %s for explicit allowlisted asset origin %s; "
                    "continuing only for asset URLs embedded in captured article HTML",
...
class Collector:
    ...
    def fetch(self, url: str, *, purpose: str, binary: bool = False) -> tuple[bytes, requests.Response]:
        if self.crawl.get("require_robots_check", True) and purpose != "robots":
            ...
            allowlisted_image_hosts = {
                str(value).lower() for value in self.site.get("image_hosts", [])
            }
            allow_asset_if_unavailable = bool(
                purpose == "image"
                and host in allowlisted_image_hosts
                and self.crawl.get(
                    "allow_explicit_asset_fetch_when_robots_unavailable", False
                )
            )
            if not self.robots.allowed(
                url, allow_if_unavailable=allow_asset_if_unavailable
            ):
    ...
    def download_image(
        ...
        try:
            ...
            # Preserve the global stop-on-401/403/429 rule for the actual asset request.
            raise
        except (PermissionError, RobotsUnavailable, requests.RequestException, ValueError) as exc:
            self.stats["images_skipped"] += 1
            self.skipped_images_log.append(
                {
                    "run_id": self.run_id,
            ...
            self.skipped_images_log.append(
                {
                    ...
                    "article_url": article_url,
                    "image_url": image_url,
                    "reason": "NonImageMimeType",
                    "detail": f"received Content-Type {mime!r}",
                }
            )
            return None
